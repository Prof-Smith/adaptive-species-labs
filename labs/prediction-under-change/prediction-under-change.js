const $=id=>document.getElementById(id);const canvas=$('forecastCanvas'),ctx=canvas.getContext('2d');let W=820,H=560,t=0,running=false,points=[],breakPoint=null,forced=false,ecosystemVisible=false;const PLOT_TOP=78,PLOT_BOTTOM=420;function labels(){const s=+$('signal').value,v=+$('volatility').value,sh=+$('shift').value;$('signalLevel').textContent=s<35?'Noisy':s<70?'Mixed':'Clear';$('volatilityLevel').textContent=v<35?'Low':v<70?'Moderate':'High';$('shiftLevel').textContent=sh<30?'None':sh<70?'Possible':'Strong';return{s,v,sh}}function displayY(y){return Math.max(PLOT_TOP+8,Math.min(PLOT_BOTTOM-8,y))}
function forecastY(x){return 315-0.12*x+18*Math.sin(x/125)}function bandWidth(){const {v,s}=labels();return 28+v*.82+(100-s)*.22}function realityY(x){const {s,v,sh}=labels();let base=forecastY(x);let noise=Math.sin(x*.09+t*.31)*(5+v*.16)+Math.cos(x*.031)*(3+v*.10);let start=370-sh*1.8;let drift=x>start?(x-start)*(sh/100)*.55:0;if(forced&&x>250)drift+=(x-250)*.70;return base+noise+drift}function reset(){t=0;points=[];breakPoint=null;forced=false;running=false;$('runButton').textContent='Run forecast';draw();updateFeedback()}function step(){const x=70+t*14;if(x>760){running=false;$('runButton').textContent='Run forecast';return}const y=realityY(x);points.push({x,y});const f=forecastY(x),bw=bandWidth();if(!breakPoint&&Math.abs(y-f)>bw){breakPoint={x,y:displayY(y),rawY:y}}t++;draw();updateFeedback()}function loop(){if(running){step();setTimeout(loop,95)}}function toggle(){running=!running;$('runButton').textContent=running?'Pause':'Run forecast';if(running)loop()}function forceShift(){forced=true;for(let i=0;i<12;i++)step()}function draw(){labels();ctx.clearRect(0,0,W,H);const bg=ctx.createLinearGradient(0,0,W,H);bg.addColorStop(0,'#eef8ff');bg.addColorStop(.6,'#fff8eb');bg.addColorStop(1,'#fff2dd');ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);drawAxes();drawBand();drawForecast();drawReality();drawBreak();drawUpdateGate();if(ecosystemVisible)drawModelEcosystem()}function drawAxes(){ctx.strokeStyle='rgba(18,78,127,.18)';ctx.lineWidth=1;for(let i=0;i<6;i++){let y=PLOT_TOP+18+i*((PLOT_BOTTOM-PLOT_TOP-36)/5);ctx.beginPath();ctx.moveTo(58,y);ctx.lineTo(760,y);ctx.stroke()}ctx.fillStyle='#52677d';ctx.font='bold 12px Arial';ctx.fillText('past pattern',70,78);ctx.fillText('future test',615,78)}function drawBand(){const bw=bandWidth();ctx.beginPath();for(let x=70;x<=760;x+=10){let y=displayY(forecastY(x)-bw);if(x===70)ctx.moveTo(x,y);else ctx.lineTo(x,y)}for(let x=760;x>=70;x-=10){let y=displayY(forecastY(x)+bw);ctx.lineTo(x,y)}ctx.closePath();ctx.fillStyle='rgba(6,182,212,.18)';ctx.fill();ctx.strokeStyle='rgba(6,182,212,.45)';ctx.lineWidth=2;ctx.stroke()}function drawForecast(){ctx.strokeStyle='#2563eb';ctx.lineWidth=4;ctx.beginPath();for(let x=70;x<=760;x+=10){let y=displayY(forecastY(x));if(x===70)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.stroke();ctx.fillStyle='#2563eb';ctx.font='bold 12px Arial';ctx.fillText('old forecast',602,displayY(forecastY(602))-18)}function drawReality(){if(points.length<2)return;for(let i=1;i<points.length;i++){const p=points[i-1],q=points[i];ctx.strokeStyle=`rgba(249,115,22,${0.30+i/points.length*.70})`;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(p.x,displayY(p.y));ctx.lineTo(q.x,displayY(q.y));ctx.stroke()}const last=points[points.length-1];ctx.fillStyle='#f59e0b';ctx.beginPath();ctx.arc(last.x,displayY(last.y),7,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=3;ctx.stroke();if(last.y>PLOT_BOTTOM-8){ctx.fillStyle='#f59e0b';ctx.font='bold 16px Arial';ctx.fillText('↓',last.x+10,PLOT_BOTTOM-2)}}function drawBreak(){if(!breakPoint)return;ctx.strokeStyle='rgba(220,38,38,.65)';ctx.lineWidth=3;ctx.setLineDash([10,8]);ctx.beginPath();ctx.moveTo(breakPoint.x,PLOT_TOP);ctx.lineTo(breakPoint.x,PLOT_BOTTOM);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#dc2626';ctx.beginPath();ctx.arc(breakPoint.x,breakPoint.y,10,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#7f1d1d';ctx.font='bold 12px Arial';ctx.fillText('break point',breakPoint.x+14,breakPoint.y-12)}function drawUpdateGate(){const gx=642,gy=368;ctx.fillStyle=breakPoint?'rgba(22,163,74,.16)':'rgba(255,255,255,.40)';ctx.strokeStyle=breakPoint?'rgba(22,163,74,.75)':'rgba(148,163,184,.55)';ctx.lineWidth=3;roundRect(gx-92,gy-36,184,72,22,true,true);ctx.textAlign='center';ctx.fillStyle=breakPoint?'#15803d':'#64748b';ctx.font='900 18px Georgia';ctx.fillText(breakPoint?'Update needed':'Monitor',gx,gy-4);ctx.font='bold 12px Arial';ctx.fillText(breakPoint?'old pattern no longer enough':'inside useful band',gx,gy+18);ctx.textAlign='left'}function roundRect(x,y,w,h,r,fill,stroke){ctx.beginPath();ctx.moveTo(x+r,y);ctx.arcTo(x+w,y,x+w,y+h,r);ctx.arcTo(x+w,y+h,x,y+h,r);ctx.arcTo(x,y+h,x,y,r);ctx.arcTo(x,y,x+w,y,r);if(fill)ctx.fill();if(stroke)ctx.stroke()}function clamp(v,min=0,max=100){return Math.max(min,Math.min(max,v))}
...
function drawModelEcosystem(){
  ...
  ctx.textAlign='left';
  ctx.fillText('Model trust',x+18,y+29);
  ctx.fillStyle='#52677d';
  ctx.font='bold 11px Arial';
  ctx.fillText('relative trust score, not probability',x+18,y+47);

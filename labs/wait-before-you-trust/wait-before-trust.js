...
function isBypassActive(){const {d,i,t}=labels();return d>68&&(i<48||t<42)}
...
function spawn(kind){particles.push({kind,age:0,max:80})}
function step(){
  ...
  spawn('task');spawn('draft');if(i>25)spawn('review');if(t>28)spawn('return');if(isBypassActive())spawn('bypass');
  draw();updateFeedback()
}
...
function draw(){
  ...
  drawBackground();drawPipeline();drawNodes();drawCheckpoint();if(isBypassActive())drawBypassBadge();drawMeters();drawParticles();
  if(pulse>0)pulse--;particles=particles.filter(p=>p.age<p.max)
}
function drawBackground(){
  haloText('WAIT workflow',410,56,'900 24px Georgia','#124e7f');
  haloText('Review the AI draft, then return learning to the human before final use',410,84,'bold 13px Arial','#52677d');
}
...
function drawPipeline(){const {d,i,t}=labels();
  drawArrow(190,226,282,226,'rgba(249,115,22,.42)',6+d*.020);
  drawArrow(424,226,438,226,'rgba(249,115,22,.38)',5);
  drawArrow(676,226,686,226,'rgba(37,99,235,.34)',5+i*.016);
  // Skill return path sits below the review card and above the meters.
  drawPolyline([[724,306],[724,370],[126,370],[126,306]],'rgba(22,163,74,.33)',5+t*.020,[8,9]);
  // Bypass path routes to a separate badge in open space, not over meters or review text.
  if(isBypassActive())drawPolyline([[426,300],[470,335],[512,394]],'rgba(220,38,38,.40)',5,[12,8]);
}
function card(x,y,w,h,color,title,sub){
  ctx.fillStyle='rgba(255,255,255,.96)';ctx.strokeStyle=color;ctx.lineWidth=4;roundRect(x,y,w,h,24,true,true);
  haloText(title,x+w/2,y+40,'900 16px Arial',color);
  haloText(sub,x+w/2,y+70,'bold 12px Arial','#52677d')
}
function drawNodes(){
  card(54,174,138,112,'#2563eb','Human','owns task');
  card(286,174,142,112,'#f59e0b','AI draft','working answer');
  card(686,174,116,112,'#124e7f','Use','final choice')
}
function drawCheckpoint(){const {i,d,t}=labels();const x=452,y=124,w=216,h=226;
  ctx.fillStyle='rgba(255,255,255,.98)';ctx.strokeStyle=pulse?'rgba(124,58,237,.94)':'rgba(124,58,237,.62)';ctx.lineWidth=pulse?6:4;roundRect(x,y,w,h,30,true,true);
  haloText('WAIT review',x+w/2,y+36,'900 17px Arial','#7c3aed');
  const rows=[['W','Workflow',d<35?'Human-owned':d<70?'Shared':'AI-heavy'],['A','Assumptions',i>55?'Named':'Hidden'],['I','Inspect',i<35?'Thin':i<70?'Partial':'Deep'],['T','Transfer',t<35?'Weak':t<70?'Emerging':'Strong']];
  rows.forEach((r,idx)=>{
    const yy=y+78+idx*33;
    ctx.fillStyle='rgba(124,58,237,.07)';roundRect(x+20,yy-18,w-40,25,12,true,false);
    haloText(r[0],x+40,yy,'900 12px Arial','#7c3aed');
    ctx.textAlign='left';ctx.font='bold 10px Arial';ctx.fillStyle='#475569';
    ctx.fillText(r[1],x+62,yy+1);
    ctx.textAlign='right';ctx.font='bold 10px Arial';ctx.fillStyle='#475569';
    ctx.fillText(r[2],x+w-28,yy+1);
  });
}
function drawBypassBadge(){
  const x=444,y=398,w=164,h=58;
  ctx.fillStyle='rgba(255,255,255,.94)';ctx.strokeStyle='rgba(220,38,38,.56)';ctx.lineWidth=4;roundRect(x,y,w,h,21,true,true);
  haloText('Bypass risk',x+w/2,y+25,'900 14px Arial','#b91c1c');
  haloText('review skipped',x+w/2,y+46,'bold 12px Arial','#52677d')
}
function drawMeters(){
  bar('product quality',quality,70,466,'#2563eb');bar('reliance risk',risk,310,466,'#dc2626');bar('human learning',learning,550,466,'#16a34a');
  haloText(cycle?'Cycle '+cycle:'No cycles yet',746,94,'900 13px Arial',cycle?'#124e7f':'#64748b','right',4)
}
...
function point(kind,t){
  if(kind==='task')return polyPoint([[190,226],[282,226]],t);
  if(kind==='draft')return polyPoint([[424,226],[438,226]],t);
  if(kind==='review')return polyPoint([[676,226],[686,226]],t);
  if(kind==='return')return polyPoint([[724,306],[724,370],[126,370],[126,306]],t);
  return polyPoint([[426,300],[470,335],[512,394]],t)
}

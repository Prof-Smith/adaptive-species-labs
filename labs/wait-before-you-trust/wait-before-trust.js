...
function haloText(txt,x,y,font,fill,align='center',w=5){ctx.textAlign=align;ctx.font=font;ctx.lineWidth=w;ctx.strokeStyle='rgba(255,255,255,.92)';ctx.strokeText(txt,x,y);ctx.fillStyle=fill;ctx.fillText(txt,x,y)}
...
function drawBackground(){haloText('WAIT workflow',410,66,'900 24px Georgia','#124e7f');haloText('Human task → AI draft → WAIT review → final use, with learning returned to the human',410,94,'bold 13px Arial','#52677d');}
...
function drawPipeline(){const {d,i,t}=labels();
  drawArrow(178,238,314,238,'rgba(249,115,22,.46)',6+d*.025);
  drawArrow(506,238,640,238,'rgba(37,99,235,.34)',6+i*.023);
  drawArrow(640,318,178,318,'rgba(22,163,74,.32)',5+t*.028,[8,9]);
  if(d>68&&(i<48||t<42)){drawArrow(506,282,646,390,'rgba(220,38,38,.50)',7,[12,8]);}
}
function card(x,y,w,h,color,title,sub){ctx.fillStyle='rgba(255,255,255,.64)';ctx.strokeStyle=color;ctx.lineWidth=4;roundRect(x,y,w,h,24,true,true);haloText(title,x+w/2,y+38,'900 16px Arial',color);haloText(sub,x+w/2,y+65,'bold 12px Arial','#52677d')}
function drawNodes(){card(54,186,132,104,'#2563eb','Human','owns task');card(302,186,138,104,'#f59e0b','AI draft','working answer');card(640,186,124,104,'#124e7f','Use','final choice');
  ctx.fillStyle='rgba(220,38,38,.08)';ctx.strokeStyle='rgba(220,38,38,.48)';ctx.lineWidth=4;roundRect(598,364,154,74,22,true,true);haloText('Bypass risk',675,394,'900 14px Arial','#b91c1c');haloText('review skipped',675,417,'bold 12px Arial','#52677d')}
function drawCheckpoint(){const {i,d,t}=labels();const x=456,y=154,w=170,h=174;ctx.fillStyle='rgba(255,255,255,.56)';ctx.strokeStyle=pulse?'rgba(124,58,237,.90)':'rgba(124,58,237,.50)';ctx.lineWidth=pulse?6:4;roundRect(x,y,w,h,28,true,true);haloText('WAIT review',x+w/2,y+30,'900 16px Arial','#7c3aed');const rows=[['W','workflow',d<65?'human-owned':'AI-heavy'],['A','assumptions',i>55?'named':'hidden'],['I','inspection',i<35?'thin':i<70?'partial':'deep'],['T','transfer',t<35?'weak':t<70?'emerging':'strong']];rows.forEach((r,idx)=>{const yy=y+62+idx*26;ctx.fillStyle='rgba(124,58,237,.07)';roundRect(x+18,yy-15,w-36,20,10,true,false);haloText(r[0],x+35,yy,'900 12px Arial','#7c3aed');ctx.textAlign='left';ctx.font='bold 11px Arial';ctx.fillStyle='#475569';ctx.fillText(r[1]+': '+r[2],x+54,yy+1);});}
function drawMeters(){bar('product quality',quality,70,456,'#2563eb');bar('reliance risk',risk,310,456,'#dc2626');bar('human learning',learning,550,456,'#16a34a');haloText(cycle?'Cycle '+cycle:'No cycles yet',746,96,'900 13px Arial',cycle?'#124e7f':'#64748b','right',4)}
...
function point(kind,t){if(kind==='task')return {x:116+(370-116)*t,y:238};if(kind==='draft')return {x:370+(542-370)*t,y:238};if(kind==='review')return {x:542+(702-542)*t,y:238};if(kind==='return')return {x:660+(124-660)*t,y:318};return {x:542+(675-542)*t,y:282+(398-282)*t}}
function drawParticles(){particles.forEach(p=>{p.age++;let t=p.age/p.max;let pt=point(p.kind,Math.min(1,t));let color=p.kind==='bypass'?'#dc2626':p.kind==='return'?'#16a34a':p.kind==='draft'?'#f59e0b':p.kind==='review'?'#7c3aed':'#2563eb';ctx.globalAlpha=1-Math.max(0,(t-.82))*4;ctx.fillStyle=color;ctx.beginPath();ctx.arc(pt.x,pt.y,p.kind==='bypass'?8:6,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1})}
function updateFeedback(){const {d,i,t}=labels();$('qualityMetric').textContent=Math.round(quality)+' / 100';$('riskMetric').textContent=Math.round(risk)+' / 100';$('transferMetric').textContent=Math.round(learning)+' / 100';
  ...
  else if(learning>62&&risk<52){$('stateName').textContent='Judgment extended';$('stateText').textContent='The workflow returns inspected output and usable skill to the human.'}

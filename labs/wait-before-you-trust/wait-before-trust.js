...
function draw(){
  ...
  drawBackground();drawPipeline();drawNodes();drawCheckpoint();drawMeters();drawParticles();
  if(pulse>0)pulse--;particles=particles.filter(p=>p.age<p.max)
}
function drawBackground(){
  haloText('WAIT workflow',410,60,'900 24px Georgia','#124e7f');
  haloText('Review the AI draft, then return learning to the human before final use',410,88,'bold 13px Arial','#52677d');
}
...
function drawPipeline(){const {d,i,t}=labels();
  // Lines are separated from labels and box interiors.
  drawArrow(190,226,282,226,'rgba(249,115,22,.42)',6+d*.020);
  drawArrow(424,226,438,226,'rgba(249,115,22,.38)',5);
  drawArrow(638,226,654,226,'rgba(37,99,235,.34)',5+i*.016);
  // Return path sits well below the review card, with padding.
  drawPolyline([[706,306],[706,372],[126,372],[126,306]],'rgba(22,163,74,.34)',5+t*.022,[8,9]);
  // Bypass path now terminates in open space below the flow and above the meters.
  if(d>68&&(i<48||t<42))drawPolyline([[430,300],[500,382],[560,410]],'rgba(220,38,38,.42)',5,[12,8]);
}
...
function drawNodes(){
  ...
  card(654,174,126,112,'#124e7f','Use','final choice');
  ctx.fillStyle='rgba(255,255,255,.92)';ctx.strokeStyle='rgba(220,38,38,.60)';ctx.lineWidth=4;roundRect(520,404,158,76,22,true,true);
  haloText('Bypass risk',599,435,'900 14px Arial','#b91c1c');
  haloText('review skipped',599,458,'bold 12px Arial','#52677d')
}
function drawCheckpoint(){const {i,d,t}=labels();const x=452,y=134,w=190,h=210;
  ...
  haloText('WAIT review',x+w/2,y+34,'900 17px Arial','#7c3aed');
  ...
  rows.forEach((r,idx)=>{
    const yy=y+72+idx*31;
    ctx.fillStyle='rgba(124,58,237,.07)';roundRect(x+18,yy-17,w-36,23,11,true,false);
    haloText(r[0],x+36,yy,'900 12px Arial','#7c3aed');
    ctx.textAlign='left';ctx.font='bold 10px Arial';ctx.fillStyle='#475569';
    // Split long wording into two safe columns so nothing bleeds past the row.
    ctx.fillText(r[1],x+56,yy+1);
    ctx.textAlign='right';ctx.font='bold 10px Arial';ctx.fillStyle='#475569';
    ctx.fillText(r[2],x+w-24,yy+1);
  });
}
function drawMeters(){
  bar('product quality',quality,70,462,'#2563eb');bar('reliance risk',risk,310,462,'#dc2626');bar('human learning',learning,550,462,'#16a34a');
...
function point(kind,t){
  ...
  if(kind==='review')return polyPoint([[638,226],[654,226]],t);
  if(kind==='return')return polyPoint([[706,306],[706,372],[126,372],[126,306]],t);
  return polyPoint([[430,300],[500,382],[560,410]],t)
}

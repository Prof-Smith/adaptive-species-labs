...
let W=820,H=560,round=0,running=false,options=[],reserve=76,renewPulse=0,renewTarget=null,decisionMoment=false;

function pressureBaseline(){return +$('commitment').value}
function decisionClock(){
  // One shared clock governs BOTH thermometer pressure and option decay.
  // Slider sets starting urgency. Rounds add deadline pressure. Late rounds accelerate.
  const base=pressureBaseline();
  const time=round*2.0;
  const acceleration=Math.max(0,round-12)**2*.58;
...
function reset(){
  round=0;running=false;reserve=76;renewPulse=0;renewTarget=null;decisionMoment=false;
  options=[
    {name:'A',x:210,y:218,life:88,value:55},
    {name:'B',x:350,y:306,life:80,value:72},
    {name:'C',x:495,y:222,life:74,value:66},
    {name:'D',x:602,y:338,life:66,value:48}
  ];
  $('runButton').textContent='Run rounds';draw();updateFeedback()
}
function viableOptions(){return options.filter(o=>o.life>=30)}
function liveOptions(){return options.filter(o=>o.life>8)}
function optionDecay(clock,cost){
  // Early clock: low decay. Rising clock: moderate decay. Forcing clock: fast decay.
  const phaseMultiplier=clock<55?0.65:clock<82?1.0:1.65;
  const baseDecay=1.0 + cost*.018;
  const clockDrag=(clock/100)*2.25;
  return (baseDecay+clockDrag)*phaseMultiplier
}
function optionLearning(uncertainty,o){
  // Uncertainty keeps exploration valuable, but cannot fully offset deadline pressure.
  return uncertainty*.021 + Math.sin((round+o.value)*.13)*.82
}
function step(){
  if(decisionMoment){draw();updateFeedback();return}
  const {u,c,clock}=labels();
  round++;
  const currentClock=decisionClock();
  options.forEach(o=>{
    const decay=optionDecay(currentClock,c);
  ...
  const targetReserve=Math.max(0,Math.min(100,viable*24 + alive*6 + u*.12 - c*.12 - currentClock*.10));
  reserve=Math.max(0,Math.min(100,reserve*.84+targetReserve*.16));
  if(currentClock>=98||round>=23){
    decisionMoment=true;running=false;$('runButton').textContent='Run rounds';
    reserve=Math.min(reserve,viableOptions().length>=2?46:20)
  }
...
function drawField(){
  ...
  haloText('Option field',410,82,'900 22px Georgia','#124e7f');haloText('one decision clock drives pressure and option decay',410,106,'bold 12px Arial','#52677d')
}
...
function drawDecisionMoment(){
  ...
  const sub=count>=2?'the clock arrived, but multiple viable moves remain':'the clock arrived with too few viable options';
...
function updateFeedback(){
  ...
  else if(decisionMoment&&viable>=2){$('stateName').textContent='Optionality paid off';$('stateText').textContent='The decision clock arrived, but the system preserved multiple viable choices.'}
  else if(decisionMoment){$('stateName').textContent='Decision forced';$('stateText').textContent='The decision clock arrived with too few viable options remaining.'}
  else if(risk==='Low'){$('stateName').textContent='Optionality preserved';$('stateText').textContent='Several useful paths remain open while the decision clock advances.'}
  else if(risk==='Moderate'){$('stateName').textContent='Options narrowing';$('stateText').textContent='The option set is shrinking as the same clock raises pressure and decay.'}
  ...
  $('whyList').innerHTML=[`Rounds completed: ${round}.`,`Clock phase: ${phase}.`,`Live options: ${alive} of ${options.length}.`,`Viable at deadline: ${viable}.`,`Decision clock: ${clock} / 100.`].map(x=>`<li>${x}</li>`).join('');
  $('moveText').textContent=decisionMoment&&viable>=2?'The deadline arrived, but optionality preserved choice. Discuss which viable path fits the revealed environment.':decisionMoment?'The deadline arrived with too little optionality. Discuss what should have been renewed earlier.':'Use renewals selectively before the same clock makes options expensive and commitment unavoidable.'
}

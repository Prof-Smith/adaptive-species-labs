...
let W=820,H=560,round=0,running=false,options=[],reserve=72,renewPulse=0,renewTarget=null,decisionMoment=false,spent=0;

function pressureBaseline(){return +$('commitment').value}
function decisionClock(){
  const base=pressureBaseline();
  const time=round*2.15;
  const acceleration=Math.max(0,round-10)**2*.70;
  return Math.min(100,base+time+acceleration)
}
function clockPhase(clock){return clock<55?'Early':clock<82?'Rising':'Forcing'}
function labels(){
...
function reset(){
  round=0;running=false;reserve=72;renewPulse=0;renewTarget=null;decisionMoment=false;spent=0;
  // Initial lives are intentionally uneven. Passive waiting should not preserve all options.
  options=[
    {name:'A',x:210,y:218,life:82,value:55,renewed:0},
    {name:'B',x:350,y:306,life:74,value:72,renewed:0},
    {name:'C',x:495,y:222,life:66,value:66,renewed:0},
    {name:'D',x:602,y:338,life:58,value:48,renewed:0}
  ];
  $('runButton').textContent='Run rounds';draw();updateFeedback()
}
function viableOptions(){return options.filter(o=>o.life>=45)}
function liveOptions(){return options.filter(o=>o.life>8)}
function optionDecay(clock,cost,o){
  // One shared clock controls both pressure and decay. Later clock phases are expensive.
  const phaseMultiplier=clock<55?1.00:clock<82?1.55:2.45;
  const baseDecay=1.9 + cost*.026;
  const clockDrag=(clock/100)*4.0;
  const maintenancePenalty=o.renewed>0 ? -0.65 : 0.55; // renewed options decay slower; neglected options decay faster
  return Math.max(1.2,(baseDecay+clockDrag+maintenancePenalty)*phaseMultiplier)
}
function optionLearning(uncertainty,o){
  // Learning is helpful but cannot cheaply preserve options as the deadline approaches.
  return uncertainty*.011 + Math.sin((round+o.value)*.13)*.45
}
function step(){
  ...
  const passiveDrain=currentClock<55?1.0:currentClock<82?2.2:4.4;
  const targetReserve=Math.max(0,Math.min(100,viable*18 + alive*4 + u*.08 - c*.20 - currentClock*.20 - passiveDrain*round*.22));
  reserve=Math.max(0,Math.min(100,reserve*.80+targetReserve*.20));
  if(currentClock>=98||round>=21){
    decisionMoment=true;running=false;$('runButton').textContent='Run rounds';
    reserve=Math.min(reserve,viableOptions().length>=2?38:14)
  }
...
function renewalCost(){const c=+$('cost').value;const clock=decisionClock();return Math.round(10+c*.08+clock*.05)}
function renew(){
  ...
  target.life=Math.min(100,target.life+34);target.renewed=4;
...
function drawField(){
  ...
  haloText('Option field',410,82,'900 22px Georgia','#124e7f');haloText('options require renewal as the decision clock rises',410,106,'bold 12px Arial','#52677d')
}
...
function drawDecisionMoment(){
  ...
  const sub=count>=2?'renewal spending preserved multiple viable moves':'passive waiting left too few viable options';
...
function updateFeedback(extra){
  ...
  if(!round){$('stateName').textContent='Portfolio ready';$('stateText').textContent='Step through rounds to see whether useful options survive without renewal spending.'}
  ...
  else if(risk==='Low'){$('stateName').textContent='Optionality preserved';$('stateText').textContent='Several viable paths remain, but decay will accelerate as the clock rises.'}
  else if(risk==='Moderate'){$('stateName').textContent='Options narrowing';$('stateText').textContent='The option set is shrinking. Renewal spending may be needed.'}
  ...
  $('moveText').textContent=decisionMoment&&viable>=2?'The deadline arrived, and active renewal preserved choice. Discuss which viable path fits the revealed environment.':decisionMoment?'The deadline arrived with too little optionality. Discuss what should have been renewed earlier.':'Renew selectively. Preserving multiple options now requires spending budget before decay accelerates.'
}

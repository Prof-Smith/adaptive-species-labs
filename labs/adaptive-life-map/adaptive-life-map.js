...
function drawCapacity(c){
  // Use the circle for the strength number only. Put the word label outside the circle.
  const r=24+c.value*.16;
  ctx.fillStyle=c.color.replace(')',',.14)').replace('rgb','rgba');ctx.beginPath();ctx.arc(c.x,c.y,r+13,0,Math.PI*2);ctx.fill();
  ctx.fillStyle=c.color;ctx.beginPath();ctx.arc(c.x,c.y,r,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle='#fff';ctx.lineWidth=4;ctx.stroke();
  haloText(Math.round(c.value),c.x,c.y+6,'900 17px Arial','#fff');
